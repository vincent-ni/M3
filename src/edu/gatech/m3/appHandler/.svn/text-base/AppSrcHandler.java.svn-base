...
public class AppSrcHandler extends AndroidAppHandler {
	...
	public boolean getIntermediateCoverage(String targetPath,
		...
		try {
			...
			this.targetTestDevice.executeShellCommand("am broadcast -a edu.gatech.m3.emma.COLLECT_COVERAGE");
			/*if (this.targetTestDevice.sendSMS("6782345628", targetCoverageFile)) {
				Thread.sleep(coverageDumpWaitTime);
				boolean retVal = this.targetTestDevice.getFileFromDevice(
						targetCoverageFile, targetPath);
				return retVal;
			}*/
			return this.targetTestDevice.getFileFromDevice(
					targetCoverageFile, targetPath);
		} catch (Exception e) {
			Logger.logException(this.targetTestDevice.getDeviceName() + ":"
					+ e.getMessage());
